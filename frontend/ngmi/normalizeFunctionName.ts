...
export function normalizeFunctionName<
  ...
  Args extends GetArgs<Abi, FunctionName>['args'],
>({
  contract,
  functionName,
  args = [] as unknown[],
  ...
  if (!args || !Array.isArray(args) || args.length === 0) return overloadFunctions[0].name

  for (const overloadFunction of overloadFunctions) {
    const matched = args.every((arg, index) => {
      const abiParameter = overloadFunction.fragment.inputs[index] as AbiParameter
