...
function RawSlider(props: RawSliderProps) {
  const { max, min, defaultValue = 0, onValue = () => {}, className } = props
  const diff = props.pseudoMax ? props.pseudoMax / props.max : 1
  ...
  useEffect(() => onValue(value * diff), [value])
  const left = useMemo(() => {
...
export function Slider({ track = false, ...props }: SliderProps) {
  ...
  const pseudoMax = props.pseudoMax || max
  const step = pseudoMax / 5
  const trackAmount = 5
  const diff = props.pseudoMax ? props.pseudoMax / max : 1
          ...
          {new Array(5).fill(0).map((_, i) => (
            <Flex key={i}>
              <Flex className="bg-ocean-300 w-[2px] h-[8px] justify-center" style={{}}>
                <Text className="absolute mt-2">
                  {i == 0 && 0}
                  {i == trackAmount - 1 && pseudoMax}
                  {i > 0 && i < trackAmount - 1 && step * i + trackAmount * diff * i}
                </Text>
              </Flex>
              {/* {i <= 3 && (
                <Flex
