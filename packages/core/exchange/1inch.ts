...
export class OneInch implements Provider {
  ...
  static factory = ({ chainId }: { chainId: number }) =>
    new OneInch({ endpoint: 'https://api.1inch.io/v5.0/10', chainId })
  ...
  public async fetchTokenList() {
    const response =
      this.cacheTokens ||
      (await axios.get<OneInchTokenListResponse>(`${this.endpoint}/tokens`)).data
  ...
  public async quote({
    ...
    const response = await axios
      .get<OneInchQuoteResponse>(this.endpoint + '/quote', {
        params: {
  ...
  public async fetchSwapData(quote: Currency, base: CurrencyAmount, walletAddress: string) {
